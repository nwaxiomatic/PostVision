            ...
            if field_name in fields:
                ...
                if isinstance(field, _TaggableManager):
                    for tags in value.split(','):
                        queryset = queryset.filter(reduce(OR, [Q(**{field_name + '__name': tag}) for tag in tags.split('*')]))
                    ...
                    queryset = queryset.filter(reduce(OR, [Q(**{field_name : tag}) for tag in value.split('*')]))
...
class ChildOfFilter(BaseFilterBackend):
    ...
    def get_page_by_id(self, request, page_id):
        return Page.objects.filter(id__in=page_id)

    def filter_queryset(self, request, queryset, view):
        if 'child_of' in request.GET:
            try:
                parent_page_id = [int(x) for x in request.GET['child_of'].split(',')]
                for x in parent_page_id:
                    assert x >= 0
            ...
            queries = []
            for x in parent_page:
                queries += queryset.child_of(x).values_list('id', flat=True)
            queryset = Page.objects.filter(id__in=queries)
...
class RestrictedChildOfFilter(ChildOfFilter):
    ...
    def get_page_by_id(self, request, page_id):
        ...
        return site_pages.filter(id__in=page_id)

class RestrictedChildOfAndParentFilter(RestrictedChildOfFilter):
    def filter_queryset(self, request, queryset, view):
        if 'child_of_and_parent' in request.GET:
            try:
                parent_page_id = [int(x) for x in request.GET['child_of_and_parent'].split(',')]
                for x in parent_page_id:
                    assert x >= 0
